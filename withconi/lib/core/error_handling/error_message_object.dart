...
class ErrorObject extends Equatable {
  ...
  static ErrorObject mapFailureToErrorMessage({required Failure failure}) {
    return failure.when(
      ...
      emptyListFailure: () =>
          const ErrorObject(title: '빈 리스트 접근 에러', message: '불러올 수 있는 정보가 없어요'),
      noSuchDataInListFailure: () =>
          const ErrorObject(title: '해당 인덱스의 데이터가 없음', message: '정보를 불러올 수 없어요'),
      ...
      naverSigninFailure: () =>
          const ErrorObject(title: '네이버 로그인 실패', message: '네이버 정보를 불러올 수 없어요'),
      appleSigninFailure: () =>
          const ErrorObject(title: '애플 로그인 실패', message: '애플 정보를 불러올 수 없어요'),
      emailSigninFailure: () =>
          const ErrorObject(title: '이메일 로그인 실패', message: '이메일 정보를 불러올 수 없어요'),
      ...
      kakaoSigninFailure: () =>
          const ErrorObject(title: '카카오 로그인 실패', message: '카카오 정보를 불러올 수 없어요'),
      ...
      locationDisabledFailure: () =>
          const ErrorObject(title: '위치 권한 허용 실패', message: '위치 권한을 허용해주세요'),
      checkVerificationFailure: () =>
          const ErrorObject(title: '인증 검증 실패', message: '인증코드 검증에 실패했습니다'),
      cameraAccessDeniedFailure: () => const ErrorObject(
          title: '카메라 엑세스 권한 없음', message: '카메라 접근 권한을 허용해주세요'),
      galleryAccessDeniedFailure: () => const ErrorObject(
          title: '갤러리 엑세스 권한 없음', message: '갤러리 접근 권한을 허용해주세요'),
    );
  }
}
